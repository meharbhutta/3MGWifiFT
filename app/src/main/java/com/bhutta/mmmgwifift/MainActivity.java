...
//                    switchON(view);
                    ...
                    if (httpServer != null) {
                        try {
                            ss.setVisibility(View.GONE);
                            pas.setVisibility(View.GONE);
                            textView.setText(getString(R.string.server_stopped));
                            httpServer.close();
                            httpServer = null;
                        } catch (IOException e) {
                            e.printStackTrace();
                    ...
                    new Thread(new Runnable() {
                        @Override
                        public void run() {
                            turnOffHotspot(mContext);
                        }
                    }).start();
                }
            }
        });
        if (intent.hasExtra(IntentViewActivity.MAIN)) {
            byte check = intent.getByteExtra(IntentViewActivity.MAIN, (byte) 0);
            if (check == (byte) 1) findViewById(R.id.startStopServer).performClick();
            else finish();
        }
    }
    ...
    private boolean turnOffHotspot(Context context) {
